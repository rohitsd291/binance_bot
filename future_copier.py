...
def get_binance_futures_balance():
    """
    Get Binance futures USDT balance using REST API.
    """
    try:
        params = {
            "timestamp": int(time.time() * 1000)
        ...
        params["signature"] = signature
        headers = {"X-MBX-APIKEY": BINANCE_API_KEY}
        resp = requests.get(f"{REST_BASE}/fapi/v2/balance", params=params, headers=headers)
        resp.raise_for_status()
        balances = resp.json()
        ...
        console.print(f"[bold red]Error getting Binance futures balance: {e}[/bold red]")
        return 0.0

def get_bitget_futures_balance():
    """
    Get Bitget futures USDT balance using CCXT.
    """
    try:
        balance = bitget.fetch_balance()
        return float(balance.get("USDT", {}).get("total", 0.0))
    except Exception as e:
        console.print(f"[bold red]Error getting Bitget futures balance: {e}[/bold red]")
        return 0.0

def calculate_proportional_amount(binance_quantity, binance_price):
    """
    Calculate proportional amount based on account balance percentages.
    """
    try:
        # Get balances
        binance_balance = get_binance_futures_balance()
        bitget_balance = get_bitget_futures_balance()
        
        if binance_balance <= 0 or bitget_balance <= 0:
            console.print(f"[bold yellow]Warning: Invalid balances - Binance: {binance_balance}, Bitget: {bitget_balance}. Using original amount.[/bold yellow]")
            return float(binance_quantity)
        
        # Calculate trade value and percentage on Binance
        binance_trade_value = float(binance_quantity) * float(binance_price)
        percentage_used = binance_trade_value / binance_balance
        
        # Calculate proportional amount for Bitget
        bitget_trade_value = bitget_balance * percentage_used
        bitget_quantity = bitget_trade_value / float(binance_price)
        
        console.print(f"[bold cyan]Proportional Trading:[/bold cyan]")
        console.print(f"  Binance Balance: {binance_balance:.2f} USDT")
        console.print(f"  Bitget Balance: {bitget_balance:.2f} USDT")
        console.print(f"  Binance Trade: {binance_trade_value:.2f} USDT ({percentage_used*100:.2f}%)")
        console.print(f"  Bitget Trade: {bitget_trade_value:.2f} USDT ({percentage_used*100:.2f}%)")
        console.print(f"  Original Quantity: {binance_quantity}")
        console.print(f"  Proportional Quantity: {bitget_quantity:.6f}")
        
        return bitget_quantity
        
    except Exception as e:
        console.print(f"[bold red]Error calculating proportional amount: {e}. Using original amount.[/bold red]")
        return float(binance_quantity)
                    ...
                    try:
                        ...
                        if data.get("e") == "ORDER_TRADE_UPDATE":
                            ...
                            if o["X"] == "FILLED":
                                ...
                                # Calculate proportional amount based on account balances
                                proportional_amount = calculate_proportional_amount(o['q'], o['ap'])
                                
                                # print(f"Placing Bitget order for {bitget_symbol} with side {side}, original amount {o['q']}, proportional amount {proportional_amount:.6f}, leverage {leverage}, margin type {margin_type_}, direction {direction}")
                                place_bitget_order(
                                    bitget=bitget,
